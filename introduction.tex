...
The bulk of such computations manifest as \emph{loop nests}, on which there are well-known optimisations such as \emph{blocking}, otherwise known as \emph{tiling}. % TODO: cite

...

[ Some comments on the problem and why it hasn't been solved ]

% TODO: Some pictures would be nice

Devito is able to perform blocking on all dimensions apart from time.
It has been previously demonstrated that extending blocking to the time dimension has the potential to reduce the run time of `some Devito stencil loops by up to 27.5\%' \textbf{[CITATION]}.
...
\begin{itemize}
	\item Implementation of skewing/tiling in Devito (hopefully accepted)
