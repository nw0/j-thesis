...
Nevertheless, it can be difficult to employ optimally.
...
As a remark, the stencils resulting from finite-difference methods tend to have high \emph{arithmetic} intensity, while tiling is used to reduce \emph{memory} pressure.\footnote{i.e.~stencils may benefit from memory reuse, since the bottleneck might occur at the CPU.}
...
% TODO: consider visualising the iteration space here, with pointy arrow diagrams
...
We claim that the minimum valid skewing factor is equal to the maximum spatial dependence~\cite{boulet98}, i.e.~in a stencil \texttt{A[t][x] = c0*A[t-1][x] + c1*A[t-1][x-1] + ... + cn*A[t-1][x-n];}, the minimum valid skewing factor is \texttt{n}.
