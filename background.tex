...
\begin{figure}[ht]
	...
	\foreach \x in {0,1,2,3,4,5}
		\fill[gray] (\t*2,\x) circle (0.1);

	\foreach \t in {0,1,2,3,4,5}
	\foreach \x in {1,2,3,4,5}
		\draw[thick,->,gray!60] (\t*2+.2,\x-.1) -- (\t*2+1.8,\x-.9);
...
Devito's process can be divided into several stages:
...
Explicitly giving the user control over time-tiling will be initial implementation attempted, although there is a lot more clarity over how large time-tiles should be, as opposed to tiling through filters.
Additionally, there is no worry that different filters require different skewing factors in Devito, as the stencil is fixed, unlike filters which may also be re-ordered.

Therefore, it may not be as crucial to use as sophisticated an auto-tuner in Devito, as kernels are applied repeatedly.
...
In this case, the separation is explicit for the user and then hidden by the auto-tuner: an important note as this similarly exposes a lower-level API, should one define an unusual filter.
...
Thanks to cut dependence analysis, the resultant trapezoids can span iterations of a time loop. It is important to note that Pochoir is a \emph{polyhedral compiler}, which operates on unions of convex polyhedra, which are more general than the hypercube-like iteration spaces described so far.
