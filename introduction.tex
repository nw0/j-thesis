...
A tool automatically performing this transformation saves considerable labour and scope to make mistakes errors, and reduces the effort required to comprehend the calculations.
...
Decoupling optimisation from specific models means that domain specialists can take advantage of optimisations seamlessly, without their own intervention, allowing them to focus their attention and expertise on their own domain.
...
Thus the desire for a compiler to save time and labour arises.
...
Time-tiling was been studied in the form this work considers since Griebl in 2004~\cite{griebl-tt}, using a \emph{space-time mapping}, analogous to the skewed tiling which we discuss later, but expressed as affine conditions specifying hyperplane partitions of the problem domain.

Time-tiling has since been studied extensively, and long implemented in polyhedral compilers such as CLooG, providing a strong theoretical foundation for this work.
...
Even so, OPS is a source-to-source compiler, operating from the stencil level down to code generation; Devito includes all these plus layers of abstraction up to the differential equations themselves.
...
\begin{itemize}
	...
	\item A novel estimator for \emph{arithmetic intensity under time-tiling} that is consistent with existing estimators for arithmetic intensity under spatial tiling.
	Bounds for both estimators and an evaluation of their accuracy.

	\item A discussion on further implementation work, considering their importance and consistency with the transformation we have implemented.
\end{itemize}
