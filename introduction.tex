...
However, the actual application of the technique beyond theoretical study has, until this work, been limited to the OPS (Oxford Parallel library for Structured mesh solvers) project~\cite{ops-main}, who presented their implementation of time-tiling in 2017~\cite{ops-tiling}.
...
\begin{itemize}
	...
	\item Demonstration of a runtime decrease of up to 45\%, and otherwise more than 20\%, compared to Devito's existing optimisations under real-world applications, in particular the stencils from the family of acoustic wave equation operators, widely used in Devito's target domain, seismic imaging.

	\item A novel estimator for \emph{arithmetic intensity under time-tiling} that extends the existing estimator for arithmetic intensity under spatial tiling in Devito.
	...
	A model to predict when performance improvement is minimal or negative.
