...
\fill[red!20!white] (0.5,2.75-.5*\x) rectangle (.75,2.5-.5*\x);
\foreach \x in {0,1,2}
\fill[red!40!white] (0.75,2.5-.5*\x) rectangle (1,2.25-.5*\x);
\fill[red!65!white] (1,2.25) rectangle (1.25,2);
\fill[red!65!white] (1,1.5) rectangle (1.25,1.75);
\fill[red] (1.25,2) rectangle (1.5,1.75);
\draw[xstep=.25cm,ystep=.25cm] (0,0) grid (4.5,3);
\draw[thick,->] (0,0) -- (4.7,0) node[right]{$t$};
\draw[thick,->] (0,0) -- (0,3.2) node[above]{$x$};

\draw[<->] (-.15,2.75) -- (-.15,2.5) node[left]{$\delta_x$};
\end{tikzpicture}
\end{center}
\end{column}
