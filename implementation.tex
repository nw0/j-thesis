...
The tiling transformation was implemented using the visitor pattern, as it could more flexibly manipulate Devito's internal representation of the loop structure.

% TODO
\paragraph{WIP} I think this section will be rather longer than the previous `Implementation', summary -- had a problem with fmax and parallelism, so had to hoist the expression out, which meant that compose\_nodes was hard to use, as the loop body was no longer a single Node.
So I wrote a visitor, which worked beautifully.
Will explain this in many words (probably no pictures).
Also find out why fmax doesn't work -- hard!
...
\begin{description}
	...
	\item[Vectorisation and time-tiling] Suspect something is going wrong, full length of inner dimension are slower than non-blockinner trials.
\end{description}
