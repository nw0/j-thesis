...
Our implementation of time-tiling is unable to deal with imperfect loop nests, although this is possible with a few more transformations~\cite{ahmed-imperfect}.
...
Further, this might present a version of Devito that diverged too far from the original, causing the basis for evaluation to be too vague.
...
While important, we felt that supporting this feature would be time-consuming and would not add significant value to our evaluation.
It would be time consuming as the most obvious issue is that it attempts to use our computed loop bounds just one line before they are defined, but since the hoist occurs long after tiling has been completed, it might require extensive changes to the visitor that it uses to hoist expressions.
...
\begin{description}
	...
	Currently, the skewing factor is user-defined parameter.
	Regardless of its source, its legality should be checked, as Devito possesses the stencil space order and can determine this as a safety check.
	...
	This does not affect evaluation as in our trials, the auto-tuned tile sizes always use the full extent of the innermost dimension, so we have merely chosen to explicitly vectorise the innermost dimension, with the complementary benefit of saving weeks of machine-time in auto-tuner trials.
	...
	This was not an issue in our evaluation each application of time-tiling was numerically verified to be correct.
